...
export default function DesignerLightingsDecorativeLightingPage() {
  const products = getAllProducts().filter((p) => {
    const s = p.scat.replace(/_/g, " ");
    return s === "Decorative Lighting" || p.scat === "Decorative Lighting" || p.scat === "Decorative_Lighting";
  });
    ...
    <main style={{ background: "var(--paper)", minHeight: "100vh" }}>
      ...
      <section style={{ position:"relative", overflow:"hidden", paddingTop:"clamp(80px,14vw,130px)", paddingBottom:"clamp(2rem,5vw,4rem)", paddingLeft:"clamp(1.25rem,6vw,5rem)", paddingRight:"clamp(1.25rem,6vw,5rem)", background:"linear-gradient(180deg, var(--paper) 0%, var(--frost) 100%)" }}>
        ...
        <div style={{ maxWidth:"1440px", margin:"0 auto", position:"relative", zIndex:1 }}>
          <nav style={{ fontFamily:"'Exo 2',sans-serif", fontSize:"0.7rem", letterSpacing:"0.18em", textTransform:"uppercase", color:"var(--text-muted-2)", marginBottom:"1.25rem", display:"flex", alignItems:"center", gap:"0.5rem", flexWrap:"wrap" }}>
            ...
            <span style={{ color:"#0EBBF0" }}>›</span>
            <Link href="/products/designer-lightings" style={{ color:"var(--text-muted)", textDecoration:"none" }}>Designer</Link>
            <span style={{ color:"#0EBBF0" }}>›</span>
            <span style={{ color:"var(--azure-deep)", fontWeight:600 }}>Decorative Lighting</span>
          </nav>
          <p className="section-label" style={{ marginBottom:"0.5rem", color:"#0EBBF0" }}>◆ DESIGNER ◆</p>
          <h1 className="section-title" style={{ fontSize:"clamp(2rem,5vw,3rem)", marginBottom:"0.75rem" }}>Decorative Lighting</h1>
          <p style={{ fontFamily:"'DM Sans',sans-serif", fontSize:"clamp(0.9rem,1.3vw,1rem)", color:"var(--text-muted)", fontStyle:"italic", maxWidth:"600px", lineHeight:1.7 }}>Chandeliers, pendants, wall lamps, floor lamps, table lamps, and decorative outdoor fixtures.</p>
        </div>
      </section>
      ...
      <section style={{ padding:"3rem clamp(1.25rem,6vw,5rem) 7rem", background:"var(--paper)" }}>
        <div style={{ maxWidth:"1440px", margin:"0 auto" }}>
          <ProductCatalog products={products} accent="#0EBBF0" title="Decorative Lighting" />
        </div>
      </section>
